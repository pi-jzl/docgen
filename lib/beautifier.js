...
module.exports = (asyncapi) => {
  ...
  _.each(asyncapi.topics, (topic, topicName) => {
    ...
    if (topic.send) {
      beautifyMessage(topic.send);
      if (topic.send.oneOf) _.each(topic.send.oneOf, beautifyMessage);
    }

    if (topic.receive) {
      beautifyMessage(topic.receive);
      if (topic.receive.oneOf) _.each(topic.receive.oneOf, beautifyMessage);
    }
